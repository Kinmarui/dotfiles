...
# Docker publishes separate repos per distro (…/linux/ubuntu vs …/linux/debian),
# each keyed by codename. Pick the one matching this host.
docker_distro="ubuntu"; is_debian && docker_distro="debian"
if [ ! -f /etc/apt/sources.list.d/docker.list ]; then
  [ -f /etc/apt/keyrings/docker.asc ] && $SUDO rm /etc/apt/keyrings/docker.asc
  $SUDO install -m 0755 -d /etc/apt/keyrings
  $SUDO wget -qO /etc/apt/keyrings/docker.asc "https://download.docker.com/linux/${docker_distro}/gpg"
  $SUDO chmod a+r /etc/apt/keyrings/docker.asc
  echo "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.asc] https://download.docker.com/linux/${docker_distro} ${OS_CODENAME} stable" \
